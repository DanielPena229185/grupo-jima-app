...
export class AgregarTiendaService {
  ...
  getRepartidores(searchParams: SearchParamsDTO): Observable<RepartidorDTO[]> {
    return this.http.get<RepartidorDTO[]>(`${enviroment.apiUrl}/repartidor`, {
      params: <any>searchParams,
    });
  }

  registrarTienda(registrarTienda: RegistrarTiendaDTO): Observable<TiendaDTO> {
    return this.http.post<TiendaDTO>(
      `${enviroment.apiUrl}/tienda`,
      registrarTienda
    );
  }

  getGramajes(): Observable<GramajeDTO[]> {
    return this.http.get<GramajeDTO[]>(`${enviroment.apiUrl}/gramaje`);
  }
}
